...
const CorpoEmpDal = {
    ...
    GetAllCorpoEmp : async (query, params, pagination) =>{
        ...
        return await corpoEmps.find(query).populate("corporate").select(params).sort(sortObject).skip(offset).limit(Number(pageSize));
    },
    ...
    DeleteCorpoEmp: async (query) => await corpoEmps.findOneAndDelete(query),
