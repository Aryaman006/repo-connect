...
const Events = () => {
    ...
    const fetchAll = async () => {
      try {
        const response = await axios.get(`${config.ApiBaseUrl}${config.GetAllEvents}`,
          {
              params: {
        ...
        setEvents(response.data.data.records);
        const {
          totalRecords,
          totalPages,
          currentPage,
        ...
        async onOk() {
          ...
          try {
            if (isAdding && !isEditing) {
              try {
                await axios.post(config.AddEvent, {
                  title: values.title,
            ...
            if (isEditing && !isAdding) {
              try {
                await axios.patch(config.EditEvent + values._id, {
                  title: values.title,
                  description: values.description,
                  date: values.date,
    ...
    const handleDelete = async (record) => {
      confirm({
        ...
        async onOk() {
          try {
            await axios.delete(config.DeleteEvent + record._id);
            notification.success({
              message: "Success",
    ...
    const handleDeleteImage = async (record, file,index) => {
      try {
          await axios.delete(`${config.DeleteEventImage}${record._id}/${index}`);
          notification.success({
