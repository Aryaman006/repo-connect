...
const UserPlanUpgradeCouponApply = () => {
  ...
  const GetPlanPrice = async () => {
    try {
      let params = {
        order_for: CONSTANTS.PAYMENT.ORDER_FOR.PLAN_UPGRADE,
      ...
      // let res = await Axios.fetchAxiosData(config.GetPlanUpgradePrice, {params});
      let res = await axios.get(config.GetPlanUpgradePrice, {params});
      ...
      if (res.status===200) {
        setPaymentPrice({
          ...
          pre_discount:res.data.data.pre_discount,
          consumed_days:res.data.data.consumed_days,
          remaining_days:res.data.data.remaining_days,
        });
      }
  ...
  const initPayment = (data) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const res = await Axios.postAxiosData(config.VerifyPlanUpgradePayment, response);
          if (res.success) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const res = await Axios.postAxiosData(config.CreatePlanUpgradeOrder, {...params});
      if (res.success) {
        if (res.data.amount) {
      ...
      <div className="row d-flex justify-content-center align-items-center" style={{ height: "100vh" }}>
        <div className="col-6 rounded-3 border border-1 mx-auto p-3 m-5" style={{ minHeight: "70%" }}>
          ...
          <div className="row border border-1 rounded-3 pt-3 mx-auto" style={{ lineHeight: "0.3", width: "100%" }}>
            ...
            <div className="col-12 d-flex">
              <div className="col-6">
                <p className="fw-lighter">Previous Balance</p>
              </div>
              <div className="col-6 text-end ">
                <p className="fw-light">&#8377;{(paymentPrice.pre_discount)?.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
            </div>
            <div className="col-12 d-flex">
              <div className="col-6">
            ...
            <div className="col-12 d-flex">
              ...
              <div className="col-6 text-end">
                <p className="fw-light">&#8377;{(paymentPrice.plan_price - paymentPrice.pre_discount - paymentPrice.coupon_discount)?.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              </div>
            </div>
            <div className="col-12 d-flex">
              <div className="col-6">
            ...
            <div className="col-12 d-flex">
              ...
              <div className="col-6 text-end">
                <p className="fw-bold">
                  &#8377;{(paymentPrice.plan_price - paymentPrice.pre_discount - paymentPrice.coupon_discount + paymentPrice.gst).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
              </div>
            </div>
          </div>
...
export default UserPlanUpgradeCouponApply;