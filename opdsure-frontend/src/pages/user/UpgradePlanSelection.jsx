...
const UpgradePlanSelection = () => {
  ...
  useEffect(()=>{
    const fetch = async () => {
      const response = await Axios.fetchAxiosData(config.GetSubscribedPlan);      
      setMembershipId(response.data.membership_options.membership_id);
      setCurrentMemberCount(response.data.membership_options.member_count);
      setCurrentCharges(response.data.membership_options.charges)
  ...
  const fetchPlanData = async () => {
    const response = await Axios.fetchAxiosData(config.GetUpgradePlan);
  ...
  const onPlanTypeChange = (value, cardIndex) => {
   ...
   const handleOk = () => {
    ...
    navigate("/user/payment/upgrade-plan", { state: { plan_values :{agreedTerms , plan_id,plan_name, selectedCard, membership_id: cardStates[selectedCard?.cardIndex]?.selectedPlan , planType: cardStates[selectedCard?.cardIndex]?.selectedPlanLabel, planPrice: cardStates[selectedCard?.cardIndex]?.selectedPlanPrice}} });
  };
            ...
            >
              <div className="container-fluid bg-light">
                <div className="row">
                  ...
                  <LeftCircleOutlined className="my-3 mx-3 fs-3 textblue" onClick={()=>navigate(-1)}/>
                  </div>
                </div>
                <div className="row mx-5" style={{ height: "100%" }}>
                  {chunk?.map((plan, cardIndex) => (
                    <div
                      key={plan?._id}
                      className="col-md-3 col-lg-3 mb-5 h-100"
                      style={{ height: "100%" }}
                    >
                      <Badge.Ribbon
                        text={
                      ...
                      >
                        ...
                        >
                          <div className="card-header bg-white border-0 rounded-4">
                            ...
                            <p className="text-start text-purple fw-bold custom-font-size d-flex align-items-center flex-wrap" style={{lineHeight:0.8}}>
                              ...
                              !cardStates[cardIndex]?.cardSelectVisibility[
                                ...
                                <span>{Math.round(plan?.membership_options[0].charges_incl_GST)?.toLocaleString("en-IN")}</span>
                              ) : (
                                <>
                                ...
                                >
                                  {plan?.membership_options
                                    ...
                                    ?.map(
                                      ...
                                      item.member_count >= currentMemberCount &&
                                        (item.charges >= currentCharges ) &&
                                      (
                                          <Option
                                            key={item.membership_id}
                                            value={item.membership_id}
                                            disabled={item.membership_id === membershipId &&
                                              plan._id === planId
                                            }
                                          >
                                            {
                                            (item.membership_id === membershipId &&
                                            plan._id === planId)?
                                            `${item.membership_label}  (Current Plan)` : item.membership_label }
                                          </Option>
                                        )
                                    )}
                                </Select>
                          ...
                          <div className="card-footer text-center bg-white rounded-4 border-0">
                            <Button
                              ...
                              disabled={
                                ( (
                                  membershipId !== 0 && !cardStates[cardIndex]?.isChecked
                                ) ||
                                (
                                  membershipId === 0 && !cardStates[cardIndex]?.isChecked && planId === plan._id
                                ) ||
                                ( cardStates[cardIndex]?.isChecked &&
          ...
          <div className="row" >
            ...
            {selectedCard && (
              <div className="col-12 ms-3">
                ...
                <h6 className="">
                  Your previous unused plan will be given as upfront discount.
                </h6>
              </div>
            )}
          </div>
...
export default UpgradePlanSelection;
